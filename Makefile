...
ifneq ("$(wildcard .env)","")
	...
	DB_HOST ?= 127.0.0.1
	DB_PORT ?= 5432
	DB_NAME ?= go_cloud_k8s_db
	DB_USER ?= go_cloud_k8s_db
...
clean:
	...
	rm -rf bin/$(APP_EXECUTABLE) coverage.out coverage-all.out ___ArrowFlightPgServer_test_go.test
...
openapi-codegen: dependencies-openapi
	oapi-codegen --old-config-style -generate types -o pkg/db/db_types.gen.go -package db api/db.yaml
	oapi-codegen --old-config-style -templates templates_oapi-codegen -generate server -o pkg/db/db_server.gen.go -package db api/db.yaml
...
xo-codegen: dependencies-xo
	xo schema schema -s public --go-pkg=db --src templates_xo -o models ${DB_DRIVER}://${DB_USER}:=@${DB_HOST}/${DB_NAME}
	#xo schema schema -s public --go-pkg=db --src templates_xo -o pkg/db ${DB_DRIVER}://${DB_USER}:=@${DB_HOST}/${DB_NAME}
